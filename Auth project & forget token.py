...
def register():
    try:
        ...
        try:
            cursor.execute(''' INSERT INTO log_data VALUES(%s,%s,%s,%s,%s)''',(name,password,phone,mail,token))
        ...
        message = {'message':'Number is not allowed in name field.'}
        resp = jsonify(message)
...
def forget():
    ...
    random_pass = ''.join(random.choices(string.digits+string.ascii_letters, k=4))
    cursor.execute( "UPDATE log_data set Token = %s WHERE mail = %s ", [random_pass,email] )
    #data = cursor.fetchone()
    mysql.connection.commit()
    ...
    msg.body = f'Hello {email}, \n\tYour new updated password token is {random_pass}.'
    send_mail.send(msg)
    return jsonify({'mail':email,'password':random_pass})


@app.route('/passwordcng/', methods = ['POST']) # Changing the password
def passwordcng():
    cursor = mysql.connection.cursor()
    email = request.json.get('mail')
    new_pass = request.json.get('new password')
    #phone = request.json.get('Phone Number')
    
    cursor.execute( "UPDATE log_data set PASSWORD = %s WHERE mail = %s " , [new_pass,email] )
    cursor.execute( "UPDATE log_data set Token = NULL WHERE mail = %s AND password = %s " , [email,new_pass] )
    ...
    if data != None:
        mysql.connection.commit()
        return jsonify({'mail':email,'message':f'{email} is not registered.'})
    return jsonify({'mail':email,'password':new_pass})
